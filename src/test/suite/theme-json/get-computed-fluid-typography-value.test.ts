...
suite(
	'getComputedFluidTypographyValue: Tests that valid font size values are parsed.',
	() => {
			...
			'generated by the test "returns clamp value when min and max font sizes are equal" of getTypographyFontSizeValue':
				{
					...
					expected:
						'clamp(30px, 1.875rem + ((1vw - 3.2px) * 1), 30px)',
				},
		};
		...
		Object.keys( dataProvider ).forEach( ( testTitle ) => {
			test( testTitle, () => {
				...
				assert.deepEqual(
					getComputedFluidTypographyValue( rawValue ),
					expected
				);
			} );
		} );
