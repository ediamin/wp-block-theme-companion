...
suite('getTypographyValueAndUnit: Tests that valid font size values are parsed.', () => {
	...
	Object.keys( dataProvider ).forEach( ( testTitle ) => {
		test( testTitle, () => {
			...
			assert.deepEqual( getTypographyValueAndUnit( rawValue ), expected );
		} );
	} );
});

suite('getTypographyValueAndUnit: Tests that invalid font size values are not parsed and trigger incorrect usage.', () => {
	...
	Object.keys( dataProvider ).forEach( ( testTitle ) => {
		test( testTitle, () => {
			...
			assert.strictEqual( getTypographyValueAndUnit( rawValue ), null );
		} );
	} );
});
