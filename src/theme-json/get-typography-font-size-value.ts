...
import type { ThemeJson, FontSizesPreset, SettingsPropertiesTypography} from '../types';
...
function getTypographyFontSizeValue(
    ...
    */
    ...
    ) ? fluidSettings.minFontSize : defaultMinimumFontSizeLimit;
    ...
    */
    if (!minimumFontSizeRaw) {
        const preferredFontSizeInPx: number = preferredSize.unit === 'px' ? preferredSize.value : preferredSize.value * 16;
        ...
        */
        ...
        if (minimumFontSizeLimitValue?.value && calculatedMinimumFontSize <= minimumFontSizeLimitValue.value) {
